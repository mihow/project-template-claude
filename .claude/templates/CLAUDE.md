# CLAUDE.md - Global AI Agent Instructions Template

Copy this to `~/.claude/CLAUDE.md` for global Claude Code settings.
...
3. **Prefer command line tools** to reduce context usage
   ...
   - **Prefer language server plugins over grep** when available (e.g., `pylsp`, `pyright`, `lsp-pyright`) for go-to-definition, find-references, and symbol search
...
When writing PR descriptions, comments, documentation, or any summary:

- Use a measured, factual tone
- Describe the actual changes made rather than making broad claims
- **Avoid the terms "comprehensive" and "production-ready"** - these are usually premature assumptions from a narrow view of the task or project
- Avoid exaggeration and words like "complete," "fully," "robust" unless objectively true
- Minimize m-dashes, excessive lists, and emojis
- A sentence is preferred when a list will look cluttered or there are multiple lists
